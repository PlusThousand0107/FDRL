...
class PolicyGradientAgent():
    def __init__(self, lr, state_dims, gamma=0.9, n_actions=10):
        self.gamma = gamma
        self.lr = lr
        self.reward_memory = []
        self.action_memory = []
    ...
    def choose_action(self, observation):
        ...
        action = action_probs.sample() # action
    ...
    def learn(self):
        ...
        for t in range(normalized_reward.shape[0]):
            ...
            for k in range(t, normalized_reward.shape[0]):
                ...
                discount *= self.gamma
            G[t] = G_sum # dicsount reward
        G = T.tensor(G, dtype=T.float).to(self.policy.device)
        loss = 0
        for g, logprob in zip(G, self.action_memory): # (discount reward, action probabilities)
            loss += -g * logprob

        self.loss_hist.append(loss.mean().data.cpu()) # loss mean to cpu

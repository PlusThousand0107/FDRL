...
class DQNAgent(object):
    ...
    def select_action(self, observation,rand_idx):
        #epsilon  = self.INITIAL_EPSILON - k * (self.INITIAL_EPSILON - self.FINAL_EPSILON) / self.max_episode
        M=observation.shape[0]
        state = T.tensor([observation],dtype=T.float).to(self.q_eval.device)
        actions = self.q_eval.forward(state)
        a_hat=T.argmax(T.squeeze(actions),dim=1).detach().cpu().numpy()#.cpu()
    ...
    def learn(self):
        ...
        q_pred = self.q_eval.forward(states)[indices, actions] #?? P+G
        q_next = self.q_next.forward(states_).max(dim=1)[0] # st+1
        ...
        self.loss_hist.append(loss.data.cpu()) # to cpu

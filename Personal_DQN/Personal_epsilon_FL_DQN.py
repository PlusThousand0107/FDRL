    ...
    for k in range(max_episode):
        ...
        if k % interval == 0: 
            ...
            if reward > max_reward:
                ...
                for n in range(num_of_agents):
                    agent=Agents_list[n]
                    agent.save_models('models_param/Personal_DQN/'+method+'/'+directory+'/'+file_name+'_index_'+str(n)+'.pth')

                global_agent.save_models('models_param/Personal_DQN/'+method+'/'+directory+'/'+file_name+'.pth')
...
np.save('npfiles/Personal_DQN/loss/'+file_name+'__loss.npy',np.array(loss_hist))
...
np.save('npfiles/Personal_DQN/mean_loss/'+file_name+'__mean_loss.npy',np.array(mean_loss))



### plt
plt.plot(loss_hist)
plt.savefig('figs/Personal_DQN/loss/'+file_name+'__loss.png')
#plt.show()
plt.close()

plt.plot(mean_loss)
plt.savefig('figs/Personal_DQN/mean_loss/'+file_name+'__mean_loss.png')
#plt.show()  
plt.close()

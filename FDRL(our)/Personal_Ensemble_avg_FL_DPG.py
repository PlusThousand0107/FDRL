...
import numpy as np
from Personal_Reinforce_Pytorch import PolicyGradientAgent
...
if __name__ == '__main__':
    ...
    global_agent = PolicyGradientAgent(lr=1e-3,state_dims=state_num,gamma=0.9,n_actions=action_num)
    ...
    method="Personal_Ensemble_avg"

    file_name=method+"_FL_DPG_Fed_AggPer_"+str(AggPer)+"__P"+str(Personal)+"_G"+str(Global)+"_("+str(Num)+")"

    directory = 'AggPer'+str(AggPer)+'_P'+str(Personal)+'_G'+str(Global)+"_("+str(Num)+")"
    path = os.path.join('models_param/DPG/'+method, directory) 
    os.mkdir(path)
    ...
    for k in range(max_episode):
        ...
        if k % AggPer == 0:
            ...
            global_dict = global_agent.policy.state_dict() # 取得全局網路
            for kd in global_dict.keys(): 
                weighted_sum = torch.zeros_like(global_dict[kd]) 

                for n in range(num_of_agents):
                         
                    weighted_sum += Agents_list[n].policy.state_dict()[kd] * (weights[n]/ total_weight)
        
                
                global_dict[kd] = weighted_sum 
            global_agent.policy.load_state_dict(global_dict) # 更新全局網路
        ...
        if k % interval == 0: 
            ...
            if reward > max_reward:
                ...
                for n in range(num_of_agents):
                    agent=Agents_list[n]
                    agent.save_model('models_param/DPG/'+method+'/'+directory+'/'+file_name+'_index_'+str(n)+'.pth')

                global_agent.save_model('models_param/DPG/'+method+'/'+directory+'/'+file_name+'.pth')

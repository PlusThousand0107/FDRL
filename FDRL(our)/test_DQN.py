...
dqn_agent=DQNAgent( gamma=0.9, lr=1e-3, n_actions=action_num, state_dim=state_num,
                 buffer_size=buffer_size,batch_size=batch_size, INITIAL_EPSILON= 0, FINAL_EPSILON=0,max_episode=4500,
                 replace=500)
...
if choice==1: # Central
    ...
    method="Ensemble_avg_replace500_init05_end4500_"
    ...
    folder='Personal_beta05_replace500_init05_end4500'
    ...
    for n in range(num_of_agents):
        Agents_list.append(DQNAgent( gamma=0.9, lr=1e-3, n_actions=action_num, state_dim=state_num,
                buffer_size=buffer_size,batch_size=batch_size, INITIAL_EPSILON= 0.0, FINAL_EPSILON=0.0,max_episode=4500,
                replace=500)) # gamma=0
    ...
    for AggPerNum in range(len(AggPer)):
        for PG in range(len(P)):
            ...
            for i in range(num_of_agents):
                Agents_list[i].load_models('test/models/DQN/'+folder+'/AggPer'+str(AggPer[AggPerNum])+'_P'+str(P[PG])+'_G'+str(G[PG])+'_('+str(N)+')/'+folder+'_FL_DQN_Fed_AggPer_'+str(AggPer[AggPerNum])+'__P'+str(P[PG])+'_G'+str(G[PG])+'_('+str(N)+')_index_'+str(n)+'.pth')

            dqn_agent.load_models('test/models/DQN/'+folder+'/AggPer'+str(AggPer[AggPerNum])+'_P'+str(P[PG])+'_G'+str(G[PG])+'_('+str(N)+')/'+folder+'_FL_DQN_Fed_AggPer_'+str(AggPer[AggPerNum])+'__P'+str(P[PG])+'_G'+str(G[PG])+'_('+str(N)+').pth')

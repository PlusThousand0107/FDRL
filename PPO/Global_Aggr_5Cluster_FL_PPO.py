...
if __name__ == '__main__':
    ...
    file_name="Global_Aggr_5Cluster_FL_PPO___Epochs_"+str(Epochs)+"_Eps_"+str(Eps)+"_AggPer_"+str(AggPer)+"_Batch_"+str(batch_size)+"_("+str(Num)+")"
    ...
    for k in range(max_episode):
        ...
        if k % AggPer == 0:
            ...
            for kd in global_dict_actor_G.keys():
                global_dict_actor_G[kd] = torch.stack([Global_agents_list[n].actor.state_dict()[kd] for n in range(5) ], 0).mean(0)
            for kd in global_dict_critic_G.keys():
                global_dict_critic_G[kd] = torch.stack([Global_agents_list[n].critic.state_dict()[kd] for n in range(5) ], 0).mean(0)

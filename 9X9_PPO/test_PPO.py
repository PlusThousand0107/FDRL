...
if choice==1: # Central
    ...
    PPO_mean=[]
    num=1
    file_name="9X9_Global_Aggr_9Cluster_FL_PPO___Epochs_5_Eps_0.05_AggPer_10_Batch_32_(1)"
    ...
    for i in range(num):
        ...
        PPO_agent.load_model('9X9_PPO/test/models/7X7_PPO/'+file_name+'/'+file_name+'_global_agentG__(0)_actor.pth')
        agent=PPO_agent
        reward_dpg_list = list()                 
        for k in range(max_episode):
            s_actor, _ = env.reset()

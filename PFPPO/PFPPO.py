...
from Personal_PPO_agent import PPOAgent
import torch
...
if __name__ == '__main__':
    ...
    global_agent = PPOAgent(epochs=Epochs,eps=Eps,state_dims=state_num,gamma=0.9,n_actions=action_num)
    ...
    file_name="PFPPO___Epochs_"+str(Epochs)+"_Eps_"+str(Eps)+"_AggPer_"+str(AggPer)+"_Batch_"+str(batch_size)+"__P"+str(Personal)+"G"+str(Global)+"_("+str(Num)+")"

    path = os.path.join('PFPPO/models_param/PFPPO/', file_name)
    os.mkdir(path)
    ...
    for k in range(max_episode):
        ...
        for i in range(int(Ns)-1):
            for n in range(num_of_agents):
                ...
                G_prob = global_agent.choose_action(s_actor_agent) #
                a_agent =  agent.add_mix_action(P_prob*Personal+G_prob*Global) #
        ...
        if k % AggPer == 0:
            ...
            global_dict_actor = global_agent.actor.state_dict()
            global_dict_critic = global_agent.critic.state_dict()

            for kd in global_dict_actor.keys():
                weighted_sum = torch.zeros_like(global_dict_actor[kd]) 
                for n in range(num_of_agents):
                    weighted_sum += Agents_list[n].actor.state_dict()[kd] * (weights[n]/ total_weight)
                global_dict_actor[kd] = weighted_sum 
                #global_dict_actor[kd] = torch.stack([Agents_list[n].actor.state_dict()[kd] for n in range(num_of_agents)], 0).mean(0)
            for kd in global_dict_critic.keys():
                weighted_sum = torch.zeros_like(global_dict_critic[kd]) 
                for n in range(num_of_agents):
                    weighted_sum += Agents_list[n].critic.state_dict()[kd] * (weights[n]/ total_weight)
                global_dict_critic[kd] = weighted_sum 

                #global_dict_critic[kd] = torch.stack([Agents_list[n].critic.state_dict()[kd] for n in range(num_of_agents)], 0).mean(0)


            global_agent.actor.load_state_dict(global_dict_actor)
            global_agent.critic.load_state_dict(global_dict_critic)

            for n in range(num_of_agents):
                Agents_list[n].actor.load_state_dict(global_agent.actor.state_dict())
                Agents_list[n].critic.load_state_dict(global_agent.critic.state_dict())
        ...
        if k % interval == 0: 
            ...
            if reward > 1.5 and k>4500:
                global_agent.save_models("PFPPO/models_param/PFPPO/"+file_name+"/"+file_name+"__("+str(cnt)+")")
                cnt+=1
...
np.save('PFPPO/npfiles/PFPPO/mean_reward/'+file_name+'__mean_reward.npy',np.array(mean_reward))
np.save('PFPPO/npfiles/PFPPO/reward/'+file_name+'__reward.npy',np.array(reward_hist))


plt.plot(reward_hist)
plt.savefig('PFPPO/figs/PFPPO/reward/'+file_name+'__reward.png')
#plt.show()
plt.close()

plt.plot(mean_reward)
plt.savefig('PFPPO/figs/PFPPO/mean_reward/'+file_name+'__mean_reward.png')
#plt.show()
plt.close()

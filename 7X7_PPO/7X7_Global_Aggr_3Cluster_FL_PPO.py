...
if __name__ == '__main__':
    ...
    file_name="7X7_Global_Aggr_3Cluster_FL_PPO___Epochs_"+str(Epochs)+"_Eps_"+str(Eps)+"_AggPer_"+str(AggPer)+"_Batch_"+str(batch_size)+"_("+str(Num)+")"

    path = os.path.join('7X7_PPO/models_param/7X7_PPO/', file_name)
    os.mkdir(path)

    cnt=0
    max_cnt=0

    n1=[0,1,2,3,4,5,6,7,13,14,20,21,27,28,34,35,41,42,43,44,45,46,47,48] # 16
    n2=[8,9,10,11,12,15,19,22,26,29,33,36,37,38,39,40] # 9
    n3=[16,17,18,23,24,25,30,31,32]
    ...
    for k in range(max_episode):
        ...
        if k % AggPer == 0:
            ...
            for kd in global_dict_actor_G.keys():
                global_dict_actor_G[kd] = torch.stack([Global_agents_list[n].actor.state_dict()[kd] for n in range(3) ], 0).mean(0)
            for kd in global_dict_critic_G.keys():
                global_dict_critic_G[kd] = torch.stack([Global_agents_list[n].critic.state_dict()[kd] for n in range(3) ], 0).mean(0)
        ...
        if k % interval == 0: 
            ...
            if reward > 1.49 and k>5700 :
                global_agent_G.save_models("7X7_PPO/models_param/7X7_PPO/"+file_name+"/"+file_name+"_global_agentG__("+str(cnt)+")")
                cnt+=1
...
np.save('7X7_PPO/npfiles/7X7_PPO/mean_reward/'+file_name+'__mean_reward.npy',np.array(mean_reward))
np.save('7X7_PPO/npfiles/7X7_PPO/reward/'+file_name+'__reward.npy',np.array(reward_hist))


plt.plot(reward_hist)
plt.savefig('7X7_PPO/figs/7X7_PPO/reward/'+file_name+'__reward.png')
#plt.show()
plt.close()

plt.plot(mean_reward)
plt.savefig('7X7_PPO/figs/7X7_PPO/mean_reward/'+file_name+'__mean_reward.png')
#plt.show()
plt.close()
